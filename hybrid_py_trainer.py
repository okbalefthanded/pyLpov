    ...
    def process(self):        
       ...
       # collect Stimulations markers and times for each paradigm
        ...
        if self.do_train:
            ...
            ssvep_epochs = eeg_epoch(self.signal, np.array([0, self.ssvep_samples-1],dtype=int), self.ssvep_stims_time)
            ssvep_predictions = []
            ...
            if self.ssvep_mode == 'sync':                
                ...
                # self.ssvep_model = train_ssvep(self.ssvep_x, self.ssvep_y)
                ...
                # ssvep_acc = self.ssvep_model.score(self.ssvep_x, self.ssvep_y)
                # print('SSVEP train acc: ', ssvep_acc)
                # ssvep_predictions = self.ssvep_model.predict(self.ssvep_x)                
                # ssvep_targets = np.array(self.ssvep_y)   

            # pickle.dump(ssvep_epochs, open('ssvep_x', 'wb'))
            # pickle.dump(self.ssvep_y, open('ssvep_y', 'wb'))
        ...
        if self.do_save:
            ...
            #
            self.ssvep_model.fit(self.ssvep_x.transpose((2,1,0)), self.ssvep_y)
            cm = confusion_matrix(self.ssvep_y, self.ssvep_model.predict(self.ssvep_x.transpose((2,1,0))))
            print('SSVEP Confusion matrix:\n ', cm)
            save_model(self.ssvep_model_path, self.ssvep_model)
            #
